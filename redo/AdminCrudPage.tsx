...
export default function AdminCrudPage({ config }: { config: CrudConfig }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res: any = await config.api.list({ pageSize: 200, ...config.listParams });
      const next = res.list || [];
      setRows(next);
      return next;
    } catch {
  ...
  const onDelete = async (row: any) => {
    try {
      ...
      if (config.api.remove && fresh.some((r: any) => r[config.rowKey] === row[config.rowKey])) {
        message.error('删除未生效：记录仍在列表中（可能是后台接口版本过旧），请稍后重试');
      } else {
        message.success('已删除');
      }
    } catch (e: any) {
